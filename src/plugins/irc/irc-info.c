...
struct t_infolist *
irc_info_infolist_irc_modelist_cb (void *data, const char *infolist_name,
                                   void *pointer, const char *arguments)
{
    struct t_infolist *ptr_infolist;
    struct t_irc_server *ptr_server;
    ...
    if (argc >= 2)
    {
        ...
        if (!pointer && (argc >= 3))
        {
            pointer = irc_modelist_search (ptr_channel, argv[2][0]);
    ...
    if (pointer && !irc_modelist_valid (ptr_channel, pointer))
        return NULL;
    ...
    if (pointer)
    {
        ...
        if (!irc_modelist_add_to_infolist (ptr_infolist, pointer))
        {
            weechat_infolist_free (ptr_infolist);
            return NULL;
        }
...
struct t_infolist *
irc_info_infolist_irc_modelist_item_cb (void *data, const char *infolist_name,
                                        void *pointer, const char *arguments)
{
    struct t_infolist *ptr_infolist;
    struct t_irc_server *ptr_server;
    ...
    if (argc >= 3)
    {
        ...
        if (!pointer && (argc >= 4))
        {
            pointer = irc_modelist_item_number (ptr_modelist, atoi(argv[3]));
    ...
    if (pointer && !irc_modelist_item_valid (ptr_modelist, pointer))
        return NULL;
    ...
    if (pointer)
    {
        ...
        if (!irc_modelist_item_add_to_infolist (ptr_infolist, pointer))
        {
            weechat_infolist_free (ptr_infolist);
            return NULL;
        }
...
irc_info_init ()
{
    ...
    weechat_hook_infolist (
        ...
        &irc_info_infolist_irc_modelist_cb, NULL);
    weechat_hook_infolist (
        "irc_modelist_item",
        ...
        &irc_info_infolist_irc_modelist_item_cb, NULL);
    weechat_hook_infolist (
        "irc_nick",
    ...
    weechat_hook_hdata (
        ...
        &irc_modelist_hdata_modelist_cb, NULL);
    weechat_hook_hdata (
        "irc_modelist_item", N_("irc modelist item"),
        &irc_modelist_hdata_item_cb, NULL);
    weechat_hook_hdata (
